...
def print_scores(title, scores, etype, include_turn_acc=True):
    ...
    print_formated_s(title, levels, '{:20}')
...
def save_csv(scores, file_path):
    levels = ['easy', 'medium', 'hard', 'extra', 'all', 'joint_all']
    headers = ['']
    for level in levels:
        headers.append(level + ' count')
        headers.append(level + ' acc')
    with open(file_path, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(headers)
        row = ['total']
        for level in levels:
            row.append(scores[level]['count'])
            row.append(scores[level]['exec'])
        writer.writerow(row)
        for db in scores['db']:
            row = [db]
            for level in levels:
                row.append(scores['db'][db][level]['count'])
                row.append(scores['db'][db][level]['exec'])
            writer.writerow(row)
...
def evaluate(gold, predict, db_dir, etype, kmaps, plug_value, keep_distinct, progress_bar_for_each_datapoint, res_csv_path, detail_json_path):
    ...
    for i, (p, g) in enumerate(zip(plist, glist)):
        ...
        for idx, pg in enumerate(zip(p, g)):
            ...
            # Add db_name into scores
            if not db_name in scores['db']:
                scores['db'][db_name] = {}
                for level in levels:
                    scores['db'][db_name][level] = {'count': 0, 'partial': {}, 'exact': 0.}
                    scores['db'][db_name][level]['exec'] = 0
                    for type_ in partial_types:
                        scores['db'][db_name][level]['partial'][type_] = {'acc': 0., 'rec': 0., 'f1': 0.,'acc_count':0,'rec_count':0}
            ...
            scores['all']['count'] += 1
            scores['db'][db_name][hardness]['count'] += 1
            scores['db'][db_name]['all']['count'] += 1
            # print(scores['all']['count'], db_name, scores['db'][db_name][hardness]['count'])
            ...
            if etype in ["all", "exec"]:
                ...
                if exec_score:
                    ...
                    scores['all']['exec'] += 1
                    scores['db'][db_name][hardness]['exec'] += 1
                    scores['db'][db_name]['all']['exec'] += 1
            ...
            if etype in ["all", "match"]:
                ...
                scores['db'][db_name][hardness]['exact'] += exact_score
                for type_ in partial_types:
                    if partial_scores[type_]['pred_total'] > 0:
                        scores[hardness]['partial'][type_]['acc'] += partial_scores[type_]['acc']
                        scores[hardness]['partial'][type_]['acc_count'] += 1
                        scores['db'][db_name][hardness]['partial'][type_]['acc'] += partial_scores[type_]['acc']
                        scores['db'][db_name][hardness]['partial'][type_]['acc_count'] += 1
                    if partial_scores[type_]['label_total'] > 0:
                        scores[hardness]['partial'][type_]['rec'] += partial_scores[type_]['rec']
                        scores[hardness]['partial'][type_]['rec_count'] += 1
                        scores['db'][db_name][hardness]['partial'][type_]['rec'] += partial_scores[type_]['rec']
                        scores['db'][db_name][hardness]['partial'][type_]['rec_count'] += 1
                    scores[hardness]['partial'][type_]['f1'] += partial_scores[type_]['f1']
                    scores['db'][db_name][hardness]['partial'][type_]['f1'] += partial_scores[type_]['f1']
                    if partial_scores[type_]['pred_total'] > 0:
                        scores['all']['partial'][type_]['acc'] += partial_scores[type_]['acc']
                        scores['all']['partial'][type_]['acc_count'] += 1
                        scores['db'][db_name]['all']['partial'][type_]['acc'] += partial_scores[type_]['acc']
                        scores['db'][db_name]['all']['partial'][type_]['acc_count'] += 1
                    if partial_scores[type_]['label_total'] > 0:
                        scores['all']['partial'][type_]['rec'] += partial_scores[type_]['rec']
                        scores['all']['partial'][type_]['rec_count'] += 1
                        scores['db'][db_name]['all']['partial'][type_]['rec'] += partial_scores[type_]['rec']
                        scores['db'][db_name]['all']['partial'][type_]['rec_count'] += 1
                    scores['all']['partial'][type_]['f1'] += partial_scores[type_]['f1']
                    scores['db'][db_name]['all']['partial'][type_]['f1'] += partial_scores[type_]['f1']
        ...
        if all(v == 1 for v in turn_scores["exec"]):
            ...
            scores['db'][db_name]['joint_all']['exec'] += 1

        if all(v == 1 for v in turn_scores["exact"]):
            scores['joint_all']['exact'] += 1
            scores['db'][db_name]['joint_all']['exact'] += 1
    ...
    print_scores('total', scores, etype, include_turn_acc=include_turn_acc)

    for db in scores['db']:
        for level in levels:
            if scores['db'][db][level]['count'] == 0:
                continue
            if etype in ["all", "exec"]:
                scores['db'][db][level]['exec'] /= scores['db'][db][level]['count']

            if etype in ["all", "match"]:
                scores['db'][db][level]['exact'] /= scores['db'][db][level]['count']
                for type_ in partial_types:
                    if scores['db'][db][level]['partial'][type_]['acc_count'] == 0:
                        scores['db'][db][level]['partial'][type_]['acc'] = 0
                    else:
                        scores['db'][db][level]['partial'][type_]['acc'] = scores['db'][db][level]['partial'][type_]['acc'] / \
                                                                scores['db'][db][level]['partial'][type_]['acc_count'] * 1.0
                    if scores['db'][db][level]['partial'][type_]['rec_count'] == 0:
                        scores['db'][db][level]['partial'][type_]['rec'] = 0
                    else:
                        scores['db'][db][level]['partial'][type_]['rec'] = scores['db'][db][level]['partial'][type_]['rec'] / \
                                                                scores['db'][db][level]['partial'][type_]['rec_count'] * 1.0
                    if scores['db'][db][level]['partial'][type_]['acc'] == 0 and scores['db'][db][level]['partial'][type_]['rec'] == 0:
                        scores['db'][db][level]['partial'][type_]['f1'] = 1
                    else:
                        scores['db'][db][level]['partial'][type_]['f1'] = \
                            2.0 * scores['db'][db][level]['partial'][type_]['acc'] * scores['db'][db][level]['partial'][type_]['rec'] / (
                            scores['db'][db][level]['partial'][type_]['rec'] + scores['db'][db][level]['partial'][type_]['acc'])
        print_scores(db, scores['db'][db], etype, include_turn_acc=include_turn_acc)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--res_csv_path', default="", type=str,
                        help='the path to save result csv file')
    parser.add_argument('--detail_json_path', default="", type=str,
                        help='the path to save detail json file')
